...
`gcdIssueId` / UPC / ISBN. Parent-linked variants are sibling `comics.ts`
rows for the live viewer (`comicFamilyKey` = series|issue|publisher). Skip
if a variant cannot share that family with a resolved parent. Orphans skipped.
