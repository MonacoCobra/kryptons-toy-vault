...
Examples:
  ...
  python3 scripts/backfill-comic-upcs-metron.py --limit 400 --delay 0.4 --min-year 2005
"""
...
def series_base_and_year(series: str) -> tuple[str, int | None]:
    ...
    # drop leading The for matching flexibility
    return base, year
...
def http_get_json(url: str, auth: str, delay: float) -> dict | None:
    if delay > 0:
        time.sleep(delay)
...
def pick_issue(results: list[dict], series_name: str, year: int, auth: str, delay: float) -> dict | None:
    ...
    for r in results:
        ...
        if yb == year:
            ...
            score += 8
        # prefer non-one-shot / matching volume singles
    ...
    # require a decent match — avoid wrong series with same issue #
    if best_score < 50:
        return None
    return best
...
def main() -> int:
    ...
    print(f"metron backfill: {len(ids)} candidates, delay={args.delay}, min_year={args.min_year}")
    ...
    for cid in ids:
        if deadline and time.time() >= deadline:
            ...
            break
        # refresh skip against disk+local
        ...
        # cap detail fetches
        hit = pick_issue(results[:12], base, year, auth, args.delay)
        if not hit:
            no_match += 1
